...
if(ptr==NULL)cout<<"cannot delete"<<endl;
		...
		if(ptr->data==value&&ptr->left==NULL){ //case 1 沒有左子樹 
			if(ptr==root){root=root->right;cout<<"Delete "<<ptr->data<<endl;delete(ptr);} //刪除點為root case1.1
			else {
			...
			if(ptr==root){root=root->left;cout<<"Delete "<<ptr->data<<endl;delete(ptr);} //刪除點為root case2.1
			else {
				parent=findparent(root,value);//指出父點位置 
